# brandguidelines.md — Budget Engineer OS: UI/UX Pro Max Design Vault
## Version: 1.0.0 | Classification: Unicorn Design System | Mode: Dark-First
...
Define the layout DNA. Budget Engineer OS uses these specific patterns:

#### Pattern A: SaaS Hero with Bento Grid
- **Use:** Dashboard home, project overview.
...
- **Divider:** 1px `border-slate-700` with subtle glow on hover.
...
--accent-primary: #06B6D4;    /* Cyan 500 — primary actions, AI highlights */
--accent-secondary: #8B5CF6;  /* Violet 500 — secondary actions, BIM links */
--accent-tertiary: #F59E0B;   /* Amber 500 — costs, warnings, financial data */
--accent-success: #10B981;    /* Emerald 500 — validation pass, under budget */
--accent-danger: #EF4444;     /* Red 500 — errors, over budget, clashes */
...
--border-focus: rgba(6, 182, 212, 0.5);  /* Cyan glow */
```
...
- **Implementation:** CSS radial gradients + pseudo-elements with `mix-blend-mode: screen`.
- **Colors:** Cyan (`#06B6D4` at 15% opacity) + Violet (`#8B5CF6` at 10% opacity) + Amber (`#F59E0B` at 5% opacity).
- **Motion:** Slow 20s drift animation (`transform: translate` + `scale`). Must respect `prefers-reduced-motion`.
- **Placement:** Behind hero sections, empty states, and loading screens. Never behind readable text without heavy overlay.
...
- **Inputs:** `bg-slate-900`, `border-slate-700`, focus state = `ring-2 ring-cyan-500/50 border-cyan-500`.
- **Buttons:**
...
- **2D CAD:** Snap cursor changes to crosshair with cyan dot. Hover highlights entity in `rgba(6, 182, 212, 0.3)`. Selection = solid cyan stroke + glow.
...
- **Spec:** Three dots, each 8px, cyan color, wave animation (scale 0.5 → 1 → 0.5). Duration 1.4s infinite.
...
- [ ] **Focus Rings:** Every interactive element has visible `:focus-visible` state (cyan ring, 2px offset).
...
- **Colors:** Cyan family for structural, Violet for MEP, Amber for finishes, Emerald for contingencies.
...
- **Spec:** Gradient fill under line (`<defs>` linearGradient from cyan-500 to transparent). Dual Y-axis if mixing units.
...
- **Spec:** Vertical timeline with nodes. User actions = cyan dot. AI actions = violet dot. System events = slate dot.
...
- **Active State:** Cyan background pill behind active tool.
...
- **Default:** Collapsed to floating action button (FAB, 56px, cyan, shadow).
...
**Font Stack:**
- **UI / Body:** `Inter` or `Geist` — weights 400, 500, 600, 700. Clean, geometric, highly legible at small sizes.
...
- **Actor Badge:** Small pill. User = cyan dot + username. AI = violet dot + "AI Agent". System = slate dot + "System".
...
/src
  ...
  /components
    ...
    /bim         // Three.js viewer, IFC loader
...
❌ **Generic blue primary buttons** — Cyan is the identity color.
