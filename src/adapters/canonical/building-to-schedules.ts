...
import type { ScheduleSet, DoorScheduleRow, WindowScheduleRow, RoomFinishRow } from '../../lib/boq/schedules'
import { extractGraphQuantities } from './buildingGraphQuantityAdapter'
import type { RoofType } from '../designToBoq'

export function generateSchedulesFromGraph(
  ...
  const qty = extractGraphQuantities(graph)
  const gfa = qty.grossFloorArea
  const floorCount = qty.floors || 1
  const footprint = qty.footprintArea || gfa / floorCount
  ...
  const roomCount = qty.roomCount || Math.max(1, graph.spaces.length)
  ...
  if (doorCount > 12) {
    doors.push({
      mark: 'D13',
      type: 'Internal', widthMm: 900, heightMm: 2100,
      qty: doorCount - 12, material: 'Timber', finish: 'Painted',
      ironmongery: 'Lever handle, latch', notes: 'Additional doors',
    })
  }
  ...
  for (let i = 0; i < Math.min(windowCount, 8); i++) {
    ...
    windows.push({
      ...
      frame: 'Aluminium', glazing: '6mm clear',
  ...
  if (windowCount > 8) {
    windows.push({
      mark: 'W09', type: 'Sliding', widthMm: 1200, heightMm: 1200,
      qty: windowCount - 8, frame: 'Aluminium', glazing: '6mm clear',
      opening: 'Side hung', notes: 'Additional windows',
    })
  }

  // ── ROOM FINISH SCHEDULE ──
  const roomFinishes: RoomFinishRow[] = []
  const wetProgrammes = new Set(['bathroom', 'ensuite', 'kitchen', 'laundry', 'pantry'])
  for (const space of spaces) {
    const isWet = wetProgrammes.has(space.programme)
    const label = space.name || space.programme
    roomFinishes.push({
      room: label.charAt(0).toUpperCase() + label.slice(1),
      floorAreaM2: space.areaM2,
  ...
  // ── SANITARY SCHEDULE ──
  const sanitary = [...Array(wetRooms)].flatMap(() => [
  ...
  const electricalPoints = [
    { point: 'Lighting', type: 'LED downlight', qty: roomCount + 2, circuit: 'L1', ratingA: 6, notes: '' },
    { point: 'Socket outlet', type: '13A switched', qty: roomCount * 2 + 2, circuit: 'S1', ratingA: 13, notes: 'With USB' },
    { point: 'Switch', type: '1-gang', qty: roomCount + 2, circuit: 'L1', ratingA: 6, notes: '' },
  ...
  for (let i = 0; i < bedrooms; i++) {
    hvac.push({
      ...
      type: 'Split unit', capacityKw: 3.5, qty: 1,
      area: `Bedroom ${i + 1}`, refrigerant: 'R32', notes: '',
    })
  }
  if (wetRooms > 0) {
    hvac.push({
      unit: 'EF-01', type: 'Extract fan', capacityKw: 0.05,
      qty: wetRooms, area: 'Wet rooms', refrigerant: 'N/A', notes: '',
    })
  }
  ...
  if (roofType === 'cgi-truss') {
    roof.push({
      section: 'Main roof', areaM2: footprint, pitchDeg: 25,
      covering: 'CGI sheets 0.6mm', insulation: '50mm fibreglass',
      structure: 'Timber trusses @ 600mm c/c', notes: 'With gutters',
    })
  } else {
    roof.push({
      section: 'Roof slab', areaM2: footprint, pitchDeg: 0,
      covering: 'Concrete slab 150mm', insulation: '50mm XPS',
      structure: 'RCC slab', notes: 'Waterproofed',
    })
  }
