...
export function PlanComparison({ designs, selectedDesignId }: PlanComparisonProps) {
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
      <h2 className="text-xl font-semibold text-[var(--text-primary)]">Plan Option Comparison</h2>
            ...
            {designs.map((design) => {
              ...
              return (
                <tr key={design.id} className={`border-b border-white/5 ${active ? 'bg-cyan-500/10' : ''}`}>
                  <td className="px-3 py-3 font-medium text-[var(--text-primary)]">{design.name}</td>
