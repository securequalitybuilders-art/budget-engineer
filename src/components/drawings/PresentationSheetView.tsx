...
function renderMasterTitleBlock(plan: PlanModel, layout: { sheetW: number; sheetH: number }): ReactNode[] {
  const tbH = 60
  const tbY = layout.sheetH - tbH
  const tbX = 0
  const today = new Date().toISOString().slice(0, 10)
  const projectName = plan.id ? `Design: ${plan.id.slice(0, 8)}` : 'Budget Engineer'

  return [
    <g key="master-title-block">
      <rect x={tbX} y={tbY} width={layout.sheetW} height={tbH} fill={PAPER} stroke={INK} strokeWidth={CAD_THIN} />
      <text x={tbX + 10} y={tbY + 14} fontSize={8} fontWeight="bold" fill={INK} fontFamily="system-ui, sans-serif">{projectName}</text>
      <text x={tbX + 10} y={tbY + 26} fontSize={7} fill={INK} fontFamily="system-ui, sans-serif">Scale: 1:100 (approx)</text>
      <text x={tbX + 10} y={tbY + 38} fontSize={7} fill={INK} fontFamily="system-ui, sans-serif">Date: {today}</text>
      <text x={tbX + 10} y={tbY + 50} fontSize={7} fill={INK} fontFamily="system-ui, sans-serif">Sheet: A1</text>
      <text x={layout.sheetW / 2} y={tbY + 20} fontSize={7} fill={INK} fontFamily="system-ui, sans-serif" textAnchor="middle" opacity={0.6}>Drawings indicative — verify with registered professionals</text>
      <text x={layout.sheetW / 2} y={tbY + 34} fontSize={7} fill={INK} fontFamily="system-ui, sans-serif" textAnchor="middle">DzeNhare OS / Budget Engineer</text>
      <text x={layout.sheetW - 10} y={tbY + tbH - 6} fontSize={6} fill={INK} fontFamily="system-ui, sans-serif" textAnchor="end" opacity={0.5}>Presentation Sheet — A1 Landscape</text>
    </g>,
  ]
}
