import { CadDocument, CadWall, CadOpening, CadBlock, CadFloor, BimMetadata, MaterialSystem } from '../domain/types';
...
/**
 * Generate a parametric multi-storey CAD document from a parsed brief.
 * The requested area is divided across floors to set the per-floor footprint,
 * stacked storeys share the X/Y origin, the entrance is on the ground floor and
 * upper floors get windows; a stair block is added when there is more than one floor.
 */
export function generateDesignFromBrief(
  ...
  // size envelope from per-floor area with a ~1.5 aspect ratio
  ...
  // bedrooms distributed across floors (at least the ground floor gets the living areas)
  ...
  for (let f = 0; f < floorCount; f++) {
    ...
    const p = `f${f + 1}`; // wall id prefix per floor
  ...
  // stair connecting floors (placed on the ground floor footprint)
  if (floorCount > 1) {
    blocks.push({
