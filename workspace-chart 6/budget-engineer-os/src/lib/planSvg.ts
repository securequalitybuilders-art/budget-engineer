// ============================================================================
// Stage 48 — Static 2D plan SVG generator (no DOM, pure string).
// Mirrors CadPlanView rendering so the dossier and standalone .svg export show
// the same drawing. Safe to run in Node (verification) and the browser.
// ============================================================================
...
import { SectionConfig } from './sectionSvg';
...
export function buildPlanSvg(cad: CadDocument, floorId?: string, titleMeta?: TitleBlockMeta, sectionMark?: SectionConfig): string {
  ...
  // Stage 56: stairwell void cast up from a stair on the floor BELOW.
  ...
  // stairwell void + trimmer framing (dashed amber opening, on the upper floor)
  for (const st of stairwells) {
    ...
    // void fill (cross-hatch feel via low-opacity fill) + dashed edge
    ...
    // diagonal to denote opening (void) below
  ...
  // perimeter dimension (overall W x D)
  ...
  // Stage 65: section-line marker (dashed line + end bubbles)
  if (sectionMark) {
    const bubble = sectionMark.axis === 'AA' ? 'A' : 'B';
    if (sectionMark.axis === 'AA') {
      // horizontal cut at a Y value → line spans full width
      ...
      // vertical cut at an X value → line spans full height
