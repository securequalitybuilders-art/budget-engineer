// ============================================================================
// Stage 47 — Currency-Aware BOQ Export (CSV + printable HTML/PDF)
// Pure string builders, no dependencies. Currency comes from the BOQ/rate card.
// ============================================================================

import { BOQ, CadDocument, ProjectRecord } from '../domain/types';
import { currencySymbol } from './currency';
import { buildPlanSvg } from './planSvg';
import { buildSectionSvg, SectionConfig } from './sectionSvg';
import { buildDrawingRegister, planSheet, sectionSheet } from './drawingRegister';
...
/** Build a currency-aware BOQ CSV string. */
...
/** Build a self-contained, brand-styled printable HTML dossier (print → PDF). */
export function buildBoqDossierHtml(boq: BOQ, cad: CadDocument, project: ProjectRecord | null, revision = 'A', sectionConfig?: SectionConfig): string {
  ...
  const revisionRows = register.flatMap((s) =>
    s.revisions.map((r) => `
    <tr>
      <td><b>${s.sheet}</b></td>
...
/** Open the dossier HTML in a new tab for printing / save-as-PDF. */
