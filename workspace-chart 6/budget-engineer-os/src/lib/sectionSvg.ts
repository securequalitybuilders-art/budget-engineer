// ============================================================================
// Stage 53/64 — Static building section SVG (no DOM, pure string).
// Stage 64 makes the cut selectable:
//   axis 'AA' : looking North — horizontal extent is X, cut plane at a Y value.
//   axis 'BB' : looking East  — horizontal extent is Y, cut plane at an X value.
// Only openings whose host wall crosses the cut plane are shown.
// ============================================================================

import { CadDocument, Vec2, MaterialSystem } from '../domain/types';
import { TitleBlockMeta, buildTitleBlock, TITLE_BLOCK_H } from './titleBlock';
...
export interface SectionConfig {
  axis: SectionAxis;
  position: number; // model coordinate of the cut plane (Y for AA, X for BB)
}
...
export function buildSectionSvg(cad: CadDocument, titleMeta?: TitleBlockMeta, config?: SectionConfig): string {
  ...
  // horizontal model coordinate of a point depends on the viewing axis
  const hOf = (p: Vec2) => (axis === 'AA' ? p.x : p.y);
  // the cut plane runs along the OTHER coordinate
  ...
  for (const floor of cad.floors) {
    ...
    // slab band with stairwell gap (gap mapped onto the current axis)
    ...
    if (wellStair) {
      // project the stair footprint onto the horizontal axis
    ...
    // external walls (left & right edges of the elevation)
    ...
    // openings whose host wall lies on/near the cut plane (within 0.6 m)
    for (const o of cad.openings.filter((o) => o.floorId === floor.id)) {
      ...
      if (Math.abs(hostPlane - cutPos) > 0.6) continue; // not on this cut
